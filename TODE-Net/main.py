...
def parse_args():
    ...
    parser.add_argument('--dataset-dir', type=str, default='/home/tcs1/data2/lyc/MSHNet-main/dataset/IRSTD-1k')
    parser.add_argument('--batch-size', type=int, default=16)
    parser.add_argument('--epochs', type=int, default=3000)
    parser.add_argument('--lr', type=float, default=0.0013)

    parser.add_argument('--warm-epoch', type=int, default=3000) 
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.optimizer = AdamW(filter(lambda p: p.requires_grad, self.model.parameters()), lr=args.lr)
        ...
        if args.mode=='train':
            if args.if_checkpoint:
                ...
                self.save_folder = '/home/tcs1/data2/lyc/MSHNet-main/weight/MSHNet-%s'%(time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time())))
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            ...
            FA, FAT, PD = self.PD_FA.get(len(self.val_loader))  ##
            ...
            if self.mode == 'train':
                if mean_IoU > self.best_iou:
                    self.best_PD  = PD[0]
                    self.true_PD  = self.best_PD*(epoch+1)
                    self.best_FAT = FAT[0]
                    self.true_FAT = self.best_FAT*(epoch+1) 
                    self.best_iou = mean_IoU 
                    torch.save(self.model.state_dict(), self.save_folder+'/weight.pkl')
                    with open(osp.join(self.save_folder, 'metric.log'), 'a') as f:
                        f.write('{} - {:04d}\t - IoU {:.4f}%\t - PD {:.4f}%\t - FAT {:.4f}%\t - FA {:.4f}\n' .
                            format(time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time())), 
                                epoch, self.best_iou* 100, self.true_PD* 100, self.true_FAT* 100, FA[0]* 1000000))   ##
                        
                    if self.best_iou* 100 > 75.9280 and self.true_PD* 100 > 97.7099:
                       torch.save(self.model.state_dict(), self.save_folder+'/best_weight.pkl')
