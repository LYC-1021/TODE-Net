...
class SLSIoULoss(nn.Module):
    ...
    def forward(self, pred_log, target,warm_epoch, epoch, with_shape=True):
        pred = torch.sigmoid(pred_log) #将模型的原始输出通过sigmoid函数转换为概率值
        smooth = 0.0
...
def LLoss(pred, target):
        ...
        for i in range(patch_size):  
            ...
            angle_loss = (4 / (np.pi**2) ) * (torch.square(torch.arctan((pred_centery) / (pred_centerx + smooth)) 
...
class AverageMeter(object):
    ...
    def update(self, val, n=1):
        ...
        self.sum += val * n
        self.count += n
        self.avg = self.sum / self.count          #持续的调用，得到目前为止的平均值
