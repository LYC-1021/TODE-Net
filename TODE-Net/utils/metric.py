...
class PD_FA():
    def __init__(self, nclass, bins, size):
        ...
        self.FAT = np.zeros(self.bins+1)  #目标级虚警
    ...
    def update(self, preds, labels):
        ...
        for iBin in range(self.bins+1):
            ...
            matched_flags = np.zeros(len(coord_image), dtype=bool)  #创建一个与coord_image等长的布尔标记数组，初始值全部为False

            for i in range(len(coord_label)):
                centroid_label = np.array(list(coord_label[i].centroid))
                for m in range(len(coord_image)):
                    if matched_flags[m]:  
                        continue  # 如果这个图像区域已经被匹配过，就跳过它
                    ...
                    if distance <= 3:
                        ...
                        matched_flags[m] = True # 标记这个图像区域为已匹配
                        #del coord_image[m]
                        break
            ...
            for i in range(len(coord_label)):
                ...
                for m in range(len(coord_image)):
                    ...
                    area_image = np.array(coord_image[m].area)        
                    if distance > 3:       ##
                        self.distance_mismatch.append(distance)
                        self.image_area_mismatch.append(area_image)
                        matched_flags[m] = True # 标记这个图像区域为已匹配
                        #del coord_image[m]    
                        break
            ...
            self.FAT[iBin]+=len(self.distance_mismatch)  ##
    ...
    def get(self,img_num):
        ...
        return Final_FA,Final_FAT,Final_PD  ##


    def reset(self):
        self.FA  = np.zeros([self.bins+1])
        self.FAT = np.zeros([self.bins+1])  ##
        self.PD  = np.zeros([self.bins+1])
